...
class ContaComLimiteTransacoes(Conta):
    LIMITE_TRANSACOES_DIARIAS = 10

    def __init__(self, numero, cliente):
        super().__init__(numero, cliente)
        self.transacoes_por_dia = defaultdict(int)
        self.historico_completo = []

    def _registrar_transacao(self, tipo, valor):
        hoje = date.today()
        if self.transacoes_por_dia[hoje] >= self.LIMITE_TRANSACOES_DIARIAS:
            print("Limite de transações diárias excedido.")
            return False
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.historico_completo.append((tipo, valor, timestamp))
        self.transacoes_por_dia[hoje] += 1
        return True

    def sacar(self, valor):
        if self._registrar_transacao("Saque", valor):
            return super().sacar(valor)

    def depositar(self, valor):
        if self._registrar_transacao("Depósito", valor):
            return super().depositar(valor)

    def extrato(self):
        print(f"Extrato da conta {self.numero}:")
        for tipo, valor, timestamp in self.historico_completo:
            print(f"{timestamp} - {tipo}: R$ {valor:.2f}")
